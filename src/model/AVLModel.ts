...
export default class AVLModel extends BSTModel {
   ...
   */
  private rebalanceAlongPath(
    path: DataNode[],
  ): Array<RotationPathInstruction<DataNode>> {
    const rotationPath: Array<RotationPathInstruction<DataNode>> = [];
    while (path.length > 0) {
      const node = path.pop();
      assert(node !== undefined, "ancestor is undefined");
      rotationPath.push(
        this.rebalance(node, path.length === 0 ? null : path[path.length - 1]),
      );
  ...
  private rebalance(
  ...
  ): RotationPathInstruction<DataNode> {
    ...
    const secondaryDescription: RotationSecondaryDescription = {
      type: "rotation",
    ...
    return {
      node,
      shapesAfterRotation: shapesAfterRotation as
        | []
        | [TreeShape<DataNode>]
  ...
  private handleRotation(
    shapesAfterRotation: Array<TreeShape<DataNode>>,
    pivot: DataNode,
    pivotParent: DataNode | null,
  ...
  private attach(
    node: DataNode,
    parent: DataNode | null,
    directionFromParent: "left" | "right" | "root",
  ): void {
