...
describe("BSTModel", () => {
  ...
  let mockBSTController: MockBSTController;
  ...
  function insertValuesAndReturnResultantAndExpectedShape(
    arrowDirection: ArrowDirection = ArrowDirection.PARENT_TO_CHILD,
  ...
  it("should return the correct inorder traversal after insertion", () => {
    const [resultantShape, expectedShape] =
      insertValuesAndReturnResultantAndExpectedShape();
    expect(resultantShape.inorderTraversal).toEqual(
      expectedShape.inorderTraversal,
    );
  });
  ...
  it("should return the correct arrows after insertion for the PREORDER ArrowDirection", () => {
    const [resultantShape, expectedShape] =
      insertValuesAndReturnResultantAndExpectedShape(ArrowDirection.PREORDER);
    expect(resultantShape.arrows).toEqual(expectedShape.arrows);
  });

  it("should return the correct arrows after insertion for the INORDER ArrowDirection", () => {
    const [resultantShape, expectedShape] =
      insertValuesAndReturnResultantAndExpectedShape(ArrowDirection.INORDER);
    expect(resultantShape.arrows).toEqual(expectedShape.arrows);
  });

  it("should return the correct arrows after insertion for the POSTORDER ArrowDirection", () => {
    const [resultantShape, expectedShape] =
      insertValuesAndReturnResultantAndExpectedShape(ArrowDirection.POSTORDER);
    expect(resultantShape.arrows).toEqual(expectedShape.arrows);
  });

  function inorderTraversalAndLayersHaveSameValues(
    shape1: TreeShape<DataNode>,
    shape2: TreeShape<DataNode>,
  ): boolean {
  ...
  it("should return the correct shape after deleting a node with no children", () => {
    ...
    assert(
      modelDeletionInformation.type === "LEQ1Child",
      "Expected deletion information to be of type LEQ1Child",
    );
  ...
  it("should return the correct shape after deleting a node with 2 children", () => {
    ...
    const modelDeletionInformation = bstModel.delete(3);
    assert(
      modelDeletionInformation.type === "2Children",
      "Expected deletion information to be of type 2Children",
    );
    const { shape } = modelDeletionInformation;
  ...
  it("should find a node correctly", () => {
    ...
    expect(
      pathsHaveSameValues(findInfo.pathFromRootToTarget, expectedPath),
    ).toBe(true);
